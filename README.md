...
5. **两阶段语义重排**：基于 ABC 抽象基类封装重排序标准接口，已适配智谱 Rerank 模型；实现「向量粗召回 + 语义精排」两阶段检索链路，显著提升片段语义匹配精度；输出结构与原向量检索层完全对齐，下游大模型生成逻辑零侵入；支持全局配置开关与接口动态传参双重控制，可按需平衡召回率与接口耗时；新增边界自动兼容（精排数大于召回数自动截断、空结果自动跳过）与全链路耗时日志，搭配接口层参数范围校验形成双层防护，运行稳定可控。
...
11. **标准化异常体系**：统一全局异常处理器，HTTP 状态码精准透传，业务异常与系统异常分层捕获，全局统一响应格式，无二次异常。
12. **单元测试体系**：覆盖文本清洗、文件读取、文本分块、向量存储、RAG服务五大模块，包含正常、边界、异常三类场景，支持回归验证。
13. **FastAPI接口服务层**：标准化 HTTP 接口封装，覆盖健康检查、知识库批量构建、单文档增量入库、单文件上传入库、知识库列表查询、知识库删除、RAG 智能问答七大业务接口；问答接口支持动态配置召回数量、相似度阈值、来源片段开关；上传接口内置格式白名单校验与临时文件自动清理；落地统一错误码体系，全局异常统一收口，标准化响应格式，自带自动接口文档，支持外部系统快速对接。
14. **RAG 底座链路打通**：已跑通「文本清洗 → 文本分块 → 向量化 → 向量入库 → 相似度检索」完整闭环。

## 质量保障
采用分层单元测试体系，覆盖工具层、存储层、业务层、客户端层全链路核心逻辑；所有外部 API 调用全程 Mock，测试离线稳定运行；新增功能严格执行全量回归验证，确保无功能退化。新增重排序边界场景专项测试，覆盖空结果跳过、数量越界自动截断、开关控制隔离等分支，核心逻辑分支覆盖率 100%。
...
| `/api/knowledge/upload` | POST | 单文件上传入库（支持txt/md格式） | 需 API Key |
