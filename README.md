...
17. **文件格式校验工具**：内置格式白名单（TXT/MD 纯文本、PDF、DOCX）、大小超限拦截、扩展名大小写不敏感校验，上传前置兜底拦截非法文件，保障服务稳定性，纯逻辑实现可全量单测覆盖。
18. **边界场景单元测试加固**：补全文本清洗、文件校验、响应工具三大纯逻辑模块边界用例，全量 42+ 用例 100% 通过，核心逻辑分支全覆盖，支持全量回归验证。
19. **批量文档入库工具**：命令行驱动的自动化知识库构建工具，支持递归扫描目标目录及子目录，自动识别 TXT、MD、PDF、DOCX 等多格式文档；基于文件二进制MD5指纹实现增量去重，重复文档自动跳过；支持 `--force` 强制全量重建模式；执行完毕输出结构化统计报告，包含扫描数、入库数、跳过数、失败数、总切片数、总耗时。
...
21. **多格式文档解析能力**：新增通用格式解析模块，原生支持 PDF、DOCX 政务常见文档格式，自动提取文本内容并完成结构化清洗，与现有文本清洗、分块、向量化全链路无缝对接。
