...
依赖：safe_read_file、clean_single_text、TextSplitter、EmbeddingError、VectorStoreError 等
"""
import os
from app.common.logger import get_logger
from app.common.exceptions import EmbeddingError, VectorStoreError, KnowledgeNotFoundError, DocumentParseError
from app.utils.text_cleaner import safe_read_file, clean_single_text
...
class KnowledgeManager:
    ...
    def build_from_dir(self, input_dir: str,  collection_name: str = None) -> dict:
        """从指定目录递归扫描所有 TXT/MD 纯文本文件，批量构建知识库
        ...
        if not txt_files:
            logger.warning(f"目录 {input_dir} 下未找到任何 TXT/MD 纯文本文件")
            return {
                "total_files": 0,
                "success_files": 0,
                "failed_files": 0,
