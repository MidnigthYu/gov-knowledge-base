"""
文本分块工具模块
提供固定长度 + 重叠区间的文本分块器，优先按段落分隔符切割，超长片段回退为字符强制切割
依赖：get_logger
"""
...
class TextSplitter:
    ...
    def __init__(
        ...
        Args:
            chunk_size: 分块最大长度
            chunk_overlap: 相邻分块重叠字符数，须小于 chunk_size
            separator: 优先使用的段落分隔符

        Raises:
            ValueError: chunk_size 非正或 chunk_overlap 不小于 chunk_size 时抛出
        """
        if chunk_size <= 0:
    ...
    def split(self, text: str) -> List[Dict]:
        ...
        Returns:
            [{ "content": str, "chunk_index": int, "start_pos": int }, ...] 分块列表
        """
